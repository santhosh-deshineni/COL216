...
struct Cache_Simulator
{
    ...
    void executeAccess(string address, char access_type,int instr){
        ...
        if (l2setisfull)
        {
            ...
            // evicting from L1 as well if present in L1 while evicting from L2
            string remaddress = removednode->ptr->tag + l2index;
            addrlen = remaddress.length();
            bitsinset = countBits(L1.sets_num);
            string remindex = remaddress.substr(addrlen - bitsinset, addrlen);
            string remtag = remaddress.substr(0, addrlen - bitsinset);
            set remset = L1.sets[binaryToLL(remindex)];
            auto p11 = present(remset.ptr, remtag);
            if(get<1>(p11)!=NULL){
                if((get<1>(p11))->ptr->dirty==true){
                    l1writeback+=1;
                    l2write+=1;
                    if(removednode->ptr->dirty==false){
                    l2writeback+=1;
                    }
                }
                remset.size-=1;
            }
            remset.ptr=get<0>(p11);
            L1.sets[binaryToLL(remindex)] = remset;
            // -----
    ...
    void runTrace(){
        ...
        for (longint i=0;i<size;i++)
        {
            ...
            executeAccess(address,access_type,i);
        }
        ...
        cout << "Total Access Time:" << accesstime << "nanosecs" << endl;
    }
};
