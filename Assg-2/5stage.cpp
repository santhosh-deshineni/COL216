// 5 Stage MIPS pipeline without bypassing
	...
	void executeCommandsPipelined()
	{
        ...
        while(true){
			...
			if(EXrun){
				...
				if(L2.command=="bne" || L2.command=="beq"){
					ret1 = (exit_code)(instructions[L2.command](*this,L2.reg1,L2.reg2, L2.reg3));
					if(Jumpto==-1){
						PCnext=L2.PC+1;
					}
					else{
					...
					int address = locateAddress(L2.reg2);
					if (address < 0){
						ret1 = (exit_code) abs(address);
					}
					else{
						L2.ALUval=address;
					}
					...
					try
					{
						L2.ALUval = registers[registerMap[L2.reg2]] + stoi(L2.reg3);
						ret1 =(exit_code) 0;
					}
					catch (std::exception &e)
					{
					...
					L2.ALUval =(int)(instructions[L2.command](*this,L2.reg1,L2.reg2, L2.reg3));
				}
				if(ret1 != SUCCESS){
			...
			if(IDrun){
                // std::cout<<"ID IS doint instruction"<<L1.command<<"\n";
				...
				if(ret != SUCCESS){
                    ...
                    if(L2.regwrite==1 && (L2.reg1==rg2 || L2.reg1==rg3) && EXrun && IDrun){
                        EXnext=-1;
                        L2next=NULL;
                        IDnext=ID;
                        L1next=&L1;
                    ...
                    else if(L3.regwrite==1 && (L3.reg1==rg2|| L3.reg1==rg3) && MEMrun && IDrun){
                        EXnext=-1;
                        L2next=NULL;
                        IDnext=ID;
                        L1next=&L1;
