// 7-9 Stage MIPS pipeline without bypassing
...
struct MIPS_Architecture
{
	...
	{
		int val= operation(registers[registerMap[r2]], registers[registerMap[r3]]);
		return val;
	}
	...
	int bOP(std::string r1, std::string r2, std::string label, std::function<bool(int, int)> comp)
	{
		...
		if (!checkRegisters({r1, r2}))
			return 1;
		Jumpto = comp(registers[registerMap[r1]], registers[registerMap[r2]]) ? address[label] : PCcurr;
		return 0;
	}
	...
	// takes a string of the form say 20($t2) and returns the string $t2 used in lw/sw instructions
	...
	// returns the associated memory address for lw/sw instructions
        ...
        while(true){
			...
			if(ALUrun){
				...
				if (L5.command == "bne" || L5.command == "beq"){
					(instructions[L5.command](*this,L5.reg1,L5.reg2, L5.reg3));
					if(Jumpto==-1){
						PCnext=L5.PC+1;
						RWnext = ALU;
						L8next = &L5;
					...
					int address = locateAddress(L5.reg2);
					L5.ALUval=address;
					DM1next = ALU;
					L6next = &L5;
				}
				else if (L5.command == "addi"){
					L5.ALUval = registers[registerMap[L5.reg2]] + stoi(L5.reg3);
					RWnext = ALU;
					L8next = &L5;
				}
				else{
					L5.ALUval =(int)(instructions[L5.command](*this,L5.reg1,L5.reg2, L5.reg3));
					RWnext = ALU;
					L8next = &L5;
				}
			}
			...
			if(RRrun){
				...
				//Data Hazards
				if(L4.command=="lw"||L4.command=="addi"){
					std::string rg2;
					if (L4.command == "lw"){
						rg2=getreg(L4.reg2);
					}
					...
					if(L5.regwrite==1 && ALUrun && rg2==L5.reg1){
						ALUnext=-1;
						RRnext=RR;
						ID2next=ID2;
						ID1next=ID1;
					...
					else if(L6.regwrite==1 && DM1run && rg2==L6.reg1){
						ALUnext=-1;
						RRnext=RR;
						ID2next=ID2;
						ID1next=ID1;
					...
					else if(L7.regwrite==1 && DM2run && rg2==L7.reg1){
						ALUnext=-1;
						RRnext=RR;
						ID2next=ID2;
						ID1next=ID1;
					...
					else if(L8.regwrite==1 && RWrun && rg2==L8.reg1){
						ALUnext=-1;
						RRnext=RR;
						ID2next=ID2;
						ID1next=ID1;
				...
				else if(L4.command !="j"){
					std::string rg1,rg2;
					if(L4.command=="sw"){
						rg1=L4.reg1;
						rg2=getreg(L4.reg2);
					}
					else if(L4.command=="beq"||L4.command=="bne"){
						rg1=L4.reg1;
						rg2=L4.reg2;
					}
					else if(L4.regwrite == 1){
						rg1=L4.reg2;
						rg2=L4.reg3;
					}
					if(L5.regwrite==1 && ALUrun && (rg2==L5.reg1 || rg1==L5.reg1)){
						ALUnext=-1;
						RRnext=RR;
						ID2next=ID2;
						ID1next=ID1;
					...
					else if(L6.regwrite==1 && DM1run && (rg2==L6.reg1 || rg1==L6.reg1)){
						ALUnext=-1;
						RRnext=RR;
						ID2next=ID2;
						ID1next=ID1;
					...
					else if(L7.regwrite==1 && DM2run && (rg2==L7.reg1 || rg1==L7.reg1)){
						ALUnext=-1;
						RRnext=RR;
						ID2next=ID2;
						ID1next=ID1;
					...
					else if(L8.regwrite==1 && RWrun && (rg2==L8.reg1||rg1==L8.reg1)){
						ALUnext=-1;
						RRnext=RR;
						ID2next=ID2;
						ID1next=ID1;
