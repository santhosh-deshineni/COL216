...
# The inputs are n, [a1,a2,....,an] and x
...
# we use $s registers for actual C code variables namely n, array pointer, x, i, j, mid
...
main:	# Print the first prompt
		...
		move $s1, $v0
		# Copy address from $s1 into $t1 which is used to represent address of current input integer
		move $t1, $s1
		# $t0 below is used as the iterator of the input for loop
		add $t0, $zero, $zero
...
loop:	# Take integer which is to be added to array
		...
		addi $t1, $t1, 4
		addi $t0, $t0, 1
		# Loop back to take next integer of array if not complete otherwise continue
		bne $s0, $t0, loop
...
loop1:	# Jump to notf which is the NOT FOUND condition if (i > j)
		slt $t2, $s4, $s3
		bne $t2, $zero, notf
		...
		sll $t3, $s5, 2
		add $t3, $s1, $t3
		# load a[mid] from memory
		lw $t4, 0($t3)
		# Check equality of a[mid] and x
		bne $t4, $s2, elseif
		j found

elseif:	# If a[mid] < x then we make i = mid + 1 and jump back to loop1.
...
else:	# Make j = mid - 1 and jump back to loop1.
		addi $s4, $s5, -1
		j loop1

notf: 	# print the fourth prompt,which is not found and jump to exit
		li $v0, 4
		la $a0, prpt4
		syscall
		j exit

found:	# print the fifth prompt, which is found followed by the corresponding index
		li $v0, 4
		la $a0, prpt5
		syscall
		li $v0, 1
